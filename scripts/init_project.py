...
本项目由 `single-cell-multiomics-analysis` 生成。所有路径、样本、参考基因组、环境和调度资源均由 `config/` 显式定义。缺失环境先通过 `tools/bootstrap_environments.py --execute` 隔离创建并验证，再执行 `tools/validate_project.py`；正式任务只能通过 `tools/submit_workflow.py` 提交。

## English

This project was generated by `single-cell-multiomics-analysis`. Paths, samples, genome resources, environments, and scheduler resources are declared under `config/`. Create and verify missing isolated environments with `tools/bootstrap_environments.py --execute`, then run `tools/validate_project.py`. Submit production jobs only through `tools/submit_workflow.py`.
""".format(project_id)
