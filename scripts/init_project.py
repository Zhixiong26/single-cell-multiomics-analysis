...
DEFAULT_ANALYSIS = {
    "scanpy": {
        ...
        "scrublet": {"enabled": True, "min_cells": 100, "expected_doublet_rate": 0.06},
        "target_sum": 10000, "hvg_flavor": "seurat", "batch_key": "batch",
...
本项目由 `single-cell-multiomics-analysis` 生成。所有路径、样本、参考基因组、环境和调度资源均由 `config/` 显式定义。缺失环境先通过 `tools/bootstrap_environments.py --execute` 隔离创建并验证，再执行 quick/full 校验；正式任务只能通过 `tools/submit_workflow.py` 提交，结果写入 `Results/runs/<run-id>/`。
