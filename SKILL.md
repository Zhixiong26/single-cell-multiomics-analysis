...
1. Read [intake and configuration](references/configuration.md), then create an intake YAML with explicit sample metadata, inputs, genome resources, environments, and scheduler site policy.
2. Generate a new project with `scripts/init_project.py`. Never generate into a non-empty directory.
3. Run `scripts/validate_project.py` before planning and again immediately before submission. Missing inputs, indexes, checksums, required executables, or route prerequisites are blockers.
4. Read the relevant workflow reference before adapting task commands: [Scanpy](references/scanpy.md), [MethSCAn](references/methscan.md), or [ALLCools/MethylVI](references/methylvi.md).
5. Build a distinct run DAG with `scripts/plan_workflow.py`. Do not reuse a run ID or result root.
6. For execution, read [scheduler and execution](references/execution.md). Submit only when the user requested execution. Every task must pass a fresh resource inspection immediately before its `sbatch`; a prior snapshot is not reusable.
7. Judge completion from validated outputs, task completion markers, and `run_summary.json`, not from submission success or Slurm state alone.
8. Update bilingual README/Report evidence with `inspect_run.py` and `update_report.py` after material progress, failure, or recovery.

## Invariants

- Treat RNA, ALLC, annotation, genome, and environment paths as read-only inputs.
